...
export default {
    ...
    effects: {
        *fromMasterKey(action, { call, put, select }) {
            const { symbol, hd_index, accountsMap } = yield select(
                ({ accountImportModel, accountsModel }) => ({
            ...
            while (1) {
                const { private_key, address, index } = yield call(
                    getAccountFromMasterKey,
                    symbol,
                    minIndex,
                );
        ...
        *fromPrivateKey({ payload }, { call, put, select }) {
            ...
            const { symbol, accountsMap } = yield select(
                ({ accountImportModel, accountsModel }) => ({
                    symbol: accountImportModel.symbol,
                    accountsMap: accountsModel.accountsMap,
                }),
        ...
        *fromLedger({ payload }, { put, select }) {
            const { index } = payload;
            const ledger_lists = yield select(
                ({ accountImportModel }) => accountImportModel.ledger_lists,
            );
        ...
        *getAccountsFromLedger(
            ...
            const { symbol, accountsMap, old_ledger_lists } = yield select(
                ({ accountImportModel, accountsModel }) => ({
                    symbol: accountImportModel.symbol,
        ...
        *importAccount(
            ...
            const { symbol, address, type, private_key, derivationIndex } = yield select(
                ({ accountImportModel }) => ({
                    symbol: accountImportModel.symbol,
