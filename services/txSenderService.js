...
const getAllBalance = async (currentAccount,options)=>{
    ...
    if(coinSymbol!==symbol){
        ...
        if(COINS[symbol].txFeeSupport){
            amount = BigNumber.max(BigNumber(0),BigNumber(balance).shiftedBy(18).minus(BigNumber(currentGasLimit).times(BigNumber(currentGasPrice).shiftedBy(9))).shiftedBy(-18)).toNumber();
        }else{
