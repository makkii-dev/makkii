...
import { PendingComponent } from '../../../components/common';
...
class ExchangeHistory extends React.Component {
    static navigationOptions = ({ navigation }) => {
        return {
            ...
            headerRight: navigation.state.params.showButton ? (
                <TouchableOpacity
                    activeOpacity={1}
                    style={{
                        height: 48,
    ...
    componentWillMount() {
        setTimeout(() => {
            ...
            dispatch(
                createAction('accountsModel/getExchangeHistory')({
...
const mapToState = ({ accountsModel, ERC20Dex }) => {
    ...
    const key = ERC20Dex.currentAccount;
    ...
    if (currentAccount) {
        ...
        exchangeHistory = Object.keys(tmp)
            .map(k => {
                return {
                    ...tmp[k],
                    hash: k,
