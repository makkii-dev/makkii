...
class HomeCenterComponent extends React.Component {
    ...
    renderFilterOptions = options => {
        ...
        const rows = (options.length - 1) / 4 + 1;
        for (let i = 0; i < rows; i++) {
            ret.push(
                ...
                >
                    {this.renderOptionsByline(options, i, cols)}
                </View>,
            );
        }
        return ret;
    ...
    renderOptionsByline = (options, line, cols) => {
        const space = 10;
        const btnWidth = (width - 40 - 40 - space * (cols - 1)) / cols;
        const ret = [];
        for (let i = line * 4; i < Math.min((line + 1) * 4, options.length); i++) {
            const currentOption = options[i];
            console.log('currentOption: ', currentOption);
            const selected = this.containsOption(currentOption);
                ...
                >
                    ...
                    <View style={{ flexDirection: 'row' }}>
                        <TextInput
                            multiline={false}
                            maxLength={15}
                            style={{ ...styles.searchStyle, marginRight: -10, paddingRight: 15 }}
...
class Home extends Component {
                    ...
                    onPress: () => {
                        this.setState(
                            ...
                            () =>
                                setTimeout(() => {
                                    dispatch(
                                        createAction('accountsModel/deleteAccounts')({
                                            keys: [key],
                                        }),
                                    );
                                }, 500),
    ...
    _renderListItem = item => {
        ...
        return (
            ...
            >
                <TouchableOpacity activeOpacity={1} onPress={() => this.goToAccountDetail(item)}>
                    <View
                        style={{
                            ...styles.accountContainerWithShadow,
                            justifyContent: 'flex-start',
                            alignItems: 'center',
                        }}
                    >
                        ...
                        <View style={{ flex: 1, paddingVertical: 10 }}>
                            ...
                            >
                                <Text
                                    style={{
                                        ...styles.accountSubTextFontStyle2,
    ...
    render() {
        ...
        return (
            <View style={{ flex: 1 }} accessibilityLabel={this.props.navigation.state.routeName}>
                ...
                >
                    <ImageBackground
                        source={require('../../../../assets/bg_vault_home.png')}
                        style={{
                            flex: 1,
