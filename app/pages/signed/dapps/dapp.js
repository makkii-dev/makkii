...
class Dapp extends Component {
    ...
    constructor(props) {
        ...
        Object.values(this.props.accounts).map(v => {
            if (v.isDefault) this.wallet = v.address;
        });
        if (!this.wallet) {
            console.log('not set makkii');
            alertOk(strings('alert_title_error'), strings('dapp_send.error_not_set_default'));
            ...
            RNFS.readFileAssets('contentScript.bundle.js', 'utf8').then(content => {
                this.setState(
                    ...
                    () => {
                        ...
                        this.invoke
                            .define('getInitState', this.getInitState)
    ...
    EthsendTransaction = txInfo => {
        ...
        return new Promise((resolve, reject) => {
            ...
            this.props.navigation.navigate('signed_dapps_send', { message, txInfo });
...
export default connect(state => {
    return { dapps: state.dappsModel, setting: state.settingsModel, accounts: state.accountsModel };
})(Dapp);
